// Matching hyperbolic sectors under u=x+y, v=x-y: the sector on x^2-y^2=1
// from (1,0) to (cosh t,sinh t) has area t/2 (left); its image sector on
...
picture left;
{
    ...
    label(left, "area $t/2$", (1.8, 0.35), orange);
...
// connecting arrow between the two panels, in the empty space above both
draw((2.6, 1.7)--(3.4, 1.7), Arrow(TeXHead));
label("$u=x+y,\ v=x-y$", (3.0, 1.7), N, fontsize(8pt));
label("area $\times\, 2$", (3.0, 1.7), S, fontsize(8pt));
