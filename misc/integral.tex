...
The connection between the net change theorem, differential equations, and Euler's method is structural.
They are different views of the same fundamental operation:
...
\[
  F(b)-F(a)=\int_a^b f(t)\,dt \quad \text{or} \quad F(b)=F(a)+\int_a^b f(t)\,dt.
\]
...
Applying the net change theorem to the unknown solution \(y\), we integrate both sides from \(t_0\) to \(t\):
\[
...
\begin{itemize}
  \item Start with the initial value \(y_0\),
...
Euler's method arises by approximating the integral with a Riemann sum.
Take a small step size \(h\). Over the interval \([t_n,t_n+h]\),
\[
  y(t_{n+1})-y(t_n)
  =
...
This is not merely inspired by integration. It \emph{is} numerical integration.
Each Euler step performs the following process:
