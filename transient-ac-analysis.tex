...
%
% ```shell
%   podman run --rm -it --userns keep-id -v "$PWD:/work:Z" -w /work registry.gitlab.com/islandoftex/images/texlive:latest latexmk -pdf transient-ac-analysis.tex
% ```
...
\begin{center}
  {\LARGE AC and Transient Analysis}\\
  A Formula Reference for Linear Circuits\\
  Inductors, Capacitors, Impedance, Charge, Flux, Voltage \& Current
\end{center}
...
\hline
$v$--$i$ law      & $v=Ri$                 & $i=C\,\dd v/\dd t$        & $v=L\,\dd i/\dd t$\\
State variable    & ---                    & $v_C$ (charge $q=Cv$)    & $i_L$ (flux $\lambda=Li$)\\
Stored energy     & $0$                    & $\tfrac12 C v^2$         & $\tfrac12 L i^2$\\
...
Quality factor $Q$ & $\dfrac{1}{R}\sqrt{\dfrac{L}{C}}$ & $R\sqrt{\dfrac{C}{L}}$\\
\hline
\end{tabular}
\end{center}
...
\[
  ...
  \qquad
  Q = V_{\mathrm{rms}} I_{\mathrm{rms}}\sin\theta \;\;(\text{reactive, \si{\var}}).
\]
...
\vfill
\begin{center}
Passive sign convention assumed throughout.
$\jj=\sqrt{-1}$, \; $\omega = 2\pi f$, \; $\ee$ is Euler's number.
\end{center}
